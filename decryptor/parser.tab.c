...
enum yysymbol_kind_t
{
  ...
  YYSYMBOL_CIPHERTEXT = 3,                 /* CIPHERTEXT  */
...
{
  "\"end of file\"", "error", "\"invalid token\"", "CIPHERTEXT",
...
#line 38 "parser.y"
               {
                ...
                int value;
                int len = strlen((yyvsp[0].str)) / 2;
                for (int i = 0; i < len; i++) {
                    sscanf((yyvsp[0].str) + 2*i, "%02x", &value);
                ...
                printf("%s\n", plaintext);
                free((yyvsp[0].str));  /* Libera la memoria asignada para el texto cifrado */
             }
